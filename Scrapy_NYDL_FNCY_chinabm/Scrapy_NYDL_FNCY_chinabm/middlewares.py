...
class JSPageMiddleware(object):
    #通过chrome请求动态网页
    def process_request(self, request, spider): #拦截请求（当然也可以使用process_response来拦截响应）
        if spider.name == "chinabmxh_info": #以爬虫名为wangyixinwen为例
            # browser = webdriver.Chrome(executable_path="D:/Temp/chromedriver.exe")
            spider.browser.get(request.url)
            import time
            time.sleep(3)
            print ("访问:{0}".format(request.url)) # 调度器：队列 采取先进后出 #经过输出结果可以知道，是先拦截页数的url,再拦截详情页的url
            # 为了不返回到下载器去下载页面（防止重复下载），利用下面的程序将其发送给spider文件
