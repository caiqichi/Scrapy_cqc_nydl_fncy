...
from Scrapy_CQC_NYDL_FNCY_china_standard.items import InfoItem
from Scrapy_CQC_NYDL_FNCY_china_standard.pipelines import MongoDBPipeline
from Scrapy_CQC_NYDL_FNCY_china_standard import upload_replace as ur
...
class ChinabzSpider(scrapy.Spider):
    ...
    def start_requests(self):
        for num in range(1, 2):
            url = "http://std.samr.gov.cn/gb/search/gbQueryPage?searchText=%E9%A3%8E%E8%83%BD&ics=&state=&ISSUE_DATE=&sortOrder=asc&pageSize=15&pageNumber={0}&_={1}".format(
                str(num), str(int(time.time()) * 1000))
