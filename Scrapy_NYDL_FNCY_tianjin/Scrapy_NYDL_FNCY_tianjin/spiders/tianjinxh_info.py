from Scrapy_NYDL_FNCY_tianjin import upload_replace as ur
from Scrapy_NYDL_FNCY_tianjin.pipelines import MongoDBPipeline
from Scrapy_NYDL_FNCY_tianjin.items import InfoItem  # 需要修改下爬虫项目名！！！
...
class TianjinxhInfoSpider(scrapy.Spider):
    name = 'tianjinxh_info'
    # allowed_domains = ['w']
    start_urls = ['http://www.twea.org.cn/?/article/',
                  'http://www.twea.org.cn/?/hyzixun/',
                  'http://www.twea.org.cn/?/zcfg/']
    web_name = '天津市新能源协会'  # 网站名称
    category = '能源电力'  # 模块
    ...
    def parse(self, response):
        li_list = response.xpath('//ul[@class="main"]/li')
        ...
        #     next_page = "http://www.twea.org.cn/?/article/page-{0}/index.html".format(num)
        #     yield scrapy.Request(url=next_page, callback=self.parse)
        #
        # for num in range(2, 103):
        #     next_page = "http://www.twea.org.cn/?/hyzixun/page-{0}/index.html".format(num)
        #     yield scrapy.Request(url=next_page, callback=self.parse)
        #
        # for num in range(2, 11):
        #     next_page = "http://www.twea.org.cn/?/zcfg/page-{0}/index.html".format(num)
        #     yield scrapy.Request(url=next_page, callback=self.parse)
    ...
    def detail_parse(self, response):
        try:
            source_author = response.xpath('//div[@class="message"]/text()').extract_first()
            source = source_author.split('作者/来源：')[1].strip().split('发表时间')[0]
            source = source if source is not None and len(source) != 0 else self.web_name
            author = source_author.split('作者/来源：')[1].strip().split('发表时间')[0]
            author = author if author is not None and len(author) != 0 else None
        ...
        # 固定模式，改下xpath即可
        content = response.xpath('//div[@id="article"]').extract()
        content = ''.join(content)  # 转化成字符串，并且去掉空格

        # 处理图片的通用格式:
        _xpath1 = '//div[@class="main"]//img/@src'  # 改下xpath即可
        ...
        contnet_url = item['content_url']
        if "zcfg" in contnet_url:
...
if __name__ == '__main__':
    os.system('scrapy crawl tianjinxh_info')  ##改下爬虫名
