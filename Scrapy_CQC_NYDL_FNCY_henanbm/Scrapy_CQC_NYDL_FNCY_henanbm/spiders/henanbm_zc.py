from Scrapy_CQC_NYDL_FNCY_henanbm import upload_replace as ur
from Scrapy_CQC_NYDL_FNCY_henanbm.pipelines import MongoDBPipeline
from Scrapy_CQC_NYDL_FNCY_henanbm.items import InfoItem  # 需要修改下爬虫项目名！！！
...
class HenanbmZcSpider(scrapy.Spider):
    name = 'henanbm_zc'
    # allowed_domains = ['www']
    start_urls = ['http://sthjt.henan.gov.cn/xxgk/hbwj/index.html']
    web_name = '河南省生态环境厅'  # 网站名称
    category = '能源电力'  # 模块
    ...
    def parse(self, response):
        ...
        for li in li_list:
            content_url = li.xpath("./span[@class='list_tit3']/a/@href").extract_first()
            content_url = response.urljoin(content_url)
            # title = li.xpath("./span[@class='list_tit3']/a/text()").extract_first()
            issue_time = li.xpath('./span[@class="list_tit4"]/text()').extract_first()
        ...
        #     next_page = "http://sthjt.henan.gov.cn/xxgk/hbwj/index_{0}.html".format(num)
        #     yield scrapy.Request(url=next_page, callback=self.parse)

    def detail_parse(self, response):
        title = response.xpath('//div[@class="zw_con"]/h1/text()').extract_first()

        source = self.web_name
        author = None

        # 固定模式，改下xpath即可
        content = response.xpath('//div[@class="centW1130"]').extract()
        ...
        # 处理附件的通用格式
        _xpath2 = '//div[@id="BodyLabel"]//a/@href'  # 改下xpath即可
...
if __name__ == '__main__':
    os.system('scrapy crawl henanbm_zc')  # 改下爬虫名
